...
public class CustomPet_Cat extends PetConfiguration {

    public CustomPet_Cat(PathFinder pathFinder){
        super(pathFinder);
    }

    public CustomPet_Cat(PathFinder pathFinder, Player player) {
        super(pathFinder, player);
    }

    @Override
    public void spawnPet(Location location) {
        this.pet = location.getWorld().spawnEntity(location, EntityType.CAT);
